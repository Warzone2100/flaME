Partial Public Class clsMap
    ...
    Private Function Read_FME(ByVal File As clsReadFile, ByRef ResultInfo As clsInterfaceOptions) As clsResult
        ...
        ElseIf Version = 3UI Or Version = 4UI Then
            ...
            If MapWidth < 1US Or MapHeight < 1US Or MapWidth > MaxMapTileSize Or MapHeight > MaxMapTileSize Then
...
LineDone:
            ...
            For A = 0 To UnitCount - 1
                ...
                If B >= 0 Then
                    If Units(A).Type.GetCode(strTemp) Then
                        Text = "        " & Units(A).ID & " " & B & " " & Quote & strTemp & Quote & " " & Units(A).PlayerNum & " " & Quote & "NONAME" & Quote & " " & Strings.FormatNumber(XYZ_int.X, 2, TriState.True, TriState.False, TriState.False) & " " & Strings.FormatNumber(XYZ_int.Y, 2, TriState.True, TriState.False, TriState.False) & " " & Strings.FormatNumber(XYZ_int.Z, 2, TriState.True, TriState.False, TriState.False) & " " & Strings.FormatNumber(0, 2, TriState.True, TriState.False, TriState.False) & " " & Strings.FormatNumber(Units(A).Rotation, 2, TriState.True, TriState.False, TriState.False) & " " & Strings.FormatNumber(0, 2, TriState.True, TriState.False, TriState.False) & EndChar
                        ByteFile.Text_Append(Text)
                    Else
    ...
    Function Write_MinimapFile(ByVal Path As String, ByVal Overwrite As Boolean) As sResult
        ...
        For Z = 0 To Texture.GetUpperBound(0)
            For X = 0 To Texture.GetUpperBound(1)
                MinimapBitmap.SetPixel(X, Z, Drawing.ColorTranslator.FromOle(OSRGB(Texture(Z, X, 0), Texture(Z, X, 1), Texture(Z, X, 2))))
            Next
        Next
    ...
    Function Write_HeightmapBMP(ByVal Path As String, ByVal Overwrite As Boolean) As sResult
        ...
        Write_HeightmapBMP = SaveBitmap(Path, Overwrite, HeightmapBitmap)
    End Function
